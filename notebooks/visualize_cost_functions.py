...
This script provides comprehensive visualization of the piecewise linear functions
and model coefficients from the CostRev3 cost model used in the batch SGD pipeline.

Usage:
    python notebooks/visualize_cost_functions.py [--case-dir path] [--checkpoint name]
    
Examples:
    # Use default case directory and latest checkpoint
    python notebooks/visualize_cost_functions.py
    
    # Specify case directory and save plots
    python notebooks/visualize_cost_functions.py --case-dir data/cases/LEMD_EGLL --save-plots
    
    # Use specific checkpoint
    python notebooks/visualize_cost_functions.py --checkpoint checkpoint_iter_50.pt
"""
...
def create_summary_table(model: CostRev3) -> pd.DataFrame:
    """Create a comprehensive summary table of model parameters."""
    summary_data = []
    
    # Beta coefficients
    summary_data.append({
        'Parameter': 'β₀ (Intercept)',
        'Value': model.beta0.item(),
        'Trainable': model.beta0.requires_grad,
        'Description': 'Base cost offset'
    })
    
    summary_data.append({
        'Parameter': 'β₁ (AC×Dist Coefficient)', 
        'Value': model.beta1.item(),
        'Trainable': model.beta1.requires_grad,
        'Description': 'Airspace charge × distance weight'
    })
    
    summary_data.append({
        'Parameter': 'β₂ (Wind Coefficient)',
        'Value': model.beta2.item(), 
        'Trainable': model.beta2.requires_grad,
        'Description': 'Wind component weight'
    })
    
    # PLM parameters
    summary_data.append({
        'Parameter': 'AC PLM Intercept',
        'Value': model.plm_ac_dist.initial_intercept.item(),
        'Trainable': model.plm_ac_dist.initial_intercept.requires_grad,
        'Description': 'AC PLM y-intercept'
    })
    
    summary_data.append({
        'Parameter': 'AC PLM First Slope',
        'Value': model.plm_ac_dist.first_slope.item(),
        'Trainable': model.plm_ac_dist.first_slope.requires_grad,
        'Description': 'AC PLM initial slope'
    })
    
    summary_data.append({
        'Parameter': 'Wind PLM Intercept',
        'Value': model.plm_wind.initial_intercept.item(),
        'Trainable': model.plm_wind.initial_intercept.requires_grad,
        'Description': 'Wind PLM y-intercept'
    })
    
    summary_data.append({
        'Parameter': 'Wind PLM First Slope',
        'Value': model.plm_wind.first_slope.item(),
        'Trainable': model.plm_wind.first_slope.requires_grad,
        'Description': 'Wind PLM initial slope'
    })
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Visualize CostRev3 piecewise linear functions and coefficients",
...
Examples:
  ...
  python notebooks/visualize_cost_functions.py --checkpoint checkpoint_iter_50.pt
    ...
    parser.add_argument("--case-dir", default="data/cases/LEMD_EGLL",
                       help="Directory containing case data (default: data/cases/LEMD_EGLL)")
    ...
    parser.add_argument("--save-csv", action="store_true", help="Save parameter summary to CSV")
    ...
    # Create and optionally save summary table
    summary_df = create_summary_table(cost_model)
    print("Parameter Summary Table:")
    print(summary_df.to_string(index=False, float_format='%.6f'))
    
    if args.save_csv:
        csv_path = "cost_model_parameters.csv"
        summary_df.to_csv(csv_path, index=False)
        print(f"\n✓ Parameter summary saved to {csv_path}")

...
def run_backward_dp(
    ...
    for generation_node_ids in tqdm(topo_generations_node_ids, desc="Topological Generations (Backward)"):
        ...
        for i in range(len(alt_u_new_batch)): # i is the index of the transition from u to v
            ...
            potential_new_V_u_ku = V_v_val + cost_uv.double() # Cost-to-go from u via v
            
            # Standard Bellman update (min convention for costs)
            # If the new path offers a lower cost (or current V is inf, as V is initialized to inf), 
            # update V and the associated state (alt, phase, eta).
            if potential_new_V_u_ku < V[u_node_idx, k_u]: # V[u_node_idx, k_u] is initialized to inf
                node_u_name = idx_to_node[u_node_idx]
                node_v_name = idx_to_node[v_node_idx]
                distance_u_v = dist_matrix[u_node_idx, v_node_idx]
                charge_u_v = ac_matrix[u_node_idx, v_node_idx]
                tailwind_u_v = tailwind_kts_batch[i]
                cost_u_v = cost_uv
                
                V[u_node_idx, k_u] = potential_new_V_u_ku

...
def tres_forward(
    ...
    max_time_overall_seconds = (
        min_time_overall_seconds + max_flight_duration_hours * 3600
    )
    num_time_bins = (
        int((max_time_overall_seconds - min_time_overall_seconds) / delta_t_seconds) + 1
    )
    ...
    climb_perf_table = get_eta_and_distance_climb(
        performance_model, origin_airport_elevation_ft=source_elevation_ft
    )
    ...
    if etto_delta_t_seconds > 0:
        max_climb_etto_bin_idx = int(
            torch.round(
                torch.tensor(actual_max_climb_time_sec / etto_delta_t_seconds)
            ).item()
        )
        max_climb_etto_bin_idx = min(
            max_climb_etto_bin_idx, num_etto_bins - 1
        )  # Clamp to max defined bins
    else:  # Avoid division by zero if etto_delta_t_seconds is 0
        max_climb_etto_bin_idx = (
            0 if actual_max_climb_time_sec == 0 else num_etto_bins - 1
        )

    print(f"eps bin for ToC: {max_climb_etto_bin_idx}")

    active_alt = torch.full(
        (num_nodes, num_time_bins, num_etto_bins),
        float("nan"),
        dtype=torch.float64,
        device=device,
    )
    active_phase = torch.full(
        (num_nodes, num_time_bins, num_etto_bins), -1, dtype=torch.long, device=device
    )
    active_eta = torch.full(
        (num_nodes, num_time_bins, num_etto_bins),
        float("nan"),
        dtype=torch.float64,
        device=device,
    )  # in seconds since midnight (ssm)

    transitions_list: List[Tuple[int, int, float, int, int, float]] = []

    dist_matrix = torch.from_numpy(dist_matrix_np).to(
    ...
    if not (
        0 <= initial_eps_idx < num_etto_bins
    ):  # Should always be true if num_etto_bins > 0
        raise ValueError(
            f"Initial ETTO bin index {initial_eps_idx} is out of bounds [0, {num_etto_bins-1}]"
        )
    ...
    active_eta[s_idx, initial_time_bin, initial_eps_idx] = float(
        takeoff_seconds_since_midnight
    )
    ...
    try:
        ...
        raise ValueError(
            "Graph is not a DAG, cannot perform topological sort for generations."
        )

    # --- 3. Main DP Loop ---
    for generation_node_ids in tqdm(
        topo_generations_node_ids, desc="Topological Generations"
    ):
        ...
        for u_node_graph_id in generation_node_ids:
            ...
            successors = list(
                graph.successors(u_node_graph_id)
            )  # Use graph_id for graph ops
            if not successors:
                continue

            for k_u in range(num_time_bins):
                for eps_u_idx in range(num_etto_bins):
                    if not torch.isinf(active_alt[u_node_idx, k_u, eps_u_idx]):
                        ...
                        if (
                            torch.isnan(current_alt_u)
                            or current_phase_u == -1
                            or torch.isnan(current_eta_u)
                        ):
                            continue

                        u_coords_tuple = (
                            graph.nodes[u_node_graph_id].get("lat"),
                            graph.nodes[u_node_graph_id].get("lon"),
                        )
                        u_coords = [
                            (
                                u_coords_tuple[0].item()
                                if isinstance(u_coords_tuple[0], torch.Tensor)
                                else float(u_coords_tuple[0])
                            ),
                            (
                                u_coords_tuple[1].item()
                                if isinstance(u_coords_tuple[1], torch.Tensor)
                                else float(u_coords_tuple[1])
                            ),
                        ]
                        ...
                        for v_node_id_succ in successors:
                            ...
                            v_coords_tuple = (
                                graph.nodes[v_node_id_succ].get("lat"),
                                graph.nodes[v_node_id_succ].get("lon"),
                            )
                            v_coords = [
                                (
                                    v_coords_tuple[0].item()
                                    if isinstance(v_coords_tuple[0], torch.Tensor)
                                    else float(v_coords_tuple[0])
                                ),
                                (
                                    v_coords_tuple[1].item()
                                    if isinstance(v_coords_tuple[1], torch.Tensor)
                                    else float(v_coords_tuple[1])
                                ),
                            ]
        ...
        coords_src_tensor = torch.tensor(
            batch_coords_src_list, dtype=torch.float64, device=device
        )
        alts_src_tensor = torch.tensor(
            batch_alts_src_list, dtype=torch.float64, device=device
        )
        eta_src_tensor = torch.tensor(
            batch_eta_src_list, dtype=torch.float64, device=device
        )  # seconds since midnight
        phase_src_tensor = torch.tensor(
            batch_phase_src_list, dtype=torch.long, device=device
        )
        coords_tgt_tensor = torch.tensor(
            batch_coords_tgt_list, dtype=torch.float64, device=device
        )
        ...
        # Process each new state and record transitions for climb and cruise
        for i in range(len(alt_v_new_batch)):
            v_node_idx = batch_v_node_indices_list[i]
            u_node_idx_for_trans = batch_u_indices_for_cost_list[i]
            ...
            clock_time_since_first_bin_sec = eta_v_new.item() - min_time_overall_seconds
            k_v = int(round(clock_time_since_first_bin_sec / delta_t_seconds))
            if not (0 <= k_v < num_time_bins):
                continue

            # ETTO bin index eps_v_idx for the successor v
            effective_etto_v_for_bin_idx_calc = min(
                elapsed_time_v_sec, actual_max_climb_time_sec
            )
            if etto_delta_t_seconds > 0:
                eps_v_idx = int(
                    round(effective_etto_v_for_bin_idx_calc / etto_delta_t_seconds)
                )
            else:
                eps_v_idx = 0
            eps_v_idx = max(0, min(eps_v_idx, num_etto_bins - 1))

            # Update reachable state info
            if (
                torch.isnan(active_alt[v_node_idx, k_v, eps_v_idx])
                or active_phase[v_node_idx, k_v, eps_v_idx] == -1
            ):
            ...
            # Store transition involving climb and cruise phases only
            phase_u = batch_phase_src_list[i]
            phase_v = (
                phase_v_new.item()
                if isinstance(phase_v_new, torch.Tensor)
                else int(phase_v_new)
            )
            if phase_u == CLIMB and phase_v in (CLIMB, CRUISE):
                transitions_list.append(
                    (
                        u_node_idx_for_trans,
                        eps_u_idx_for_trans,
                        round(batch_alts_src_list[i], 0),
                        v_node_idx,
                        eps_v_idx,
                        round(alt_v_new.item(), 0),
                    )
                )

    return active_eta, active_alt, active_phase, transitions_list
...
def save_transitions(
    ...
    with open(os.path.join(output_dir, f"{base_filename}.pkl"), "wb") as f:
        ...
        print(
            f"Saved transitions to {os.path.join(output_dir, f'{base_filename}.pkl')}"
        )

...
from sdrizzle.route.backward_state_f1 import get_next_state_bw
from sdrizzle.route.forward_state import CLIMB, CRUISE, DESCENT
from sdrizzle.route.get_wind import get_wind
from sdrizzle.vnav.vnav_performance import Performance, get_eta_and_distance_climb, get_eta_and_distance_descent
from sdrizzle.helpers.datetimeh import datestr_to_seconds_since_midnight
from sdrizzle.wind.wind_model import WindModel
...
def _display_input_summary(
    ...
    table = Table(title="Input Summary - Backward Pass", show_header=True, header_style="bold magenta")
    table.add_column("Parameter", style="cyan", no_wrap=True)
    table.add_column("Value", style="green")
    
    table.add_row("Graph Nodes", str(len(graph.nodes())))
    table.add_row("Graph Edges", str(len(graph.edges())))
    table.add_row("Goal Node", goal_node_id)
    table.add_row("Landing Time", estimated_landing_time_str)
    table.add_row("Takeoff Time", eta_takeoff_str)
    table.add_row("Max Flight Duration", f"{max_flight_duration_hours} hours")
    table.add_row("Time Discretization", f"{delta_t_seconds_wall_clock} seconds")
    table.add_row("Climb Time Discretization", f"{delta_t_seconds_climb} seconds")
    table.add_row("Transitions Available", str(len(transitions_list)))
    
    console.print(table)


def _display_output_summary(feasible_transitions_list: list, processing_time: float):
    """Display a summary of output statistics."""
    if not feasible_transitions_list:
        console.print("[red]No feasible transitions found![/red]")
        return
    
    # Analyze transitions by phase
    phase_counts = {"CLIMB": 0, "CRUISE": 0, "DESCENT": 0}
    unique_nodes = set()
    altitude_ranges = []
    ...
    for transition in feasible_transitions_list:
        ...
        if 0 <= phase_from < len(phase_names):
            phase_counts[phase_names[phase_from]] += 1
        
        # Collect unique nodes and altitudes
        unique_nodes.add(transition[0])
        unique_nodes.add(transition[5])
        altitude_ranges.extend([alt_from, alt_to])
    
    altitude_ranges = [alt for alt in altitude_ranges if not np.isnan(alt)]
    
    table = Table(title="Output Summary - Feasible Transitions", show_header=True, header_style="bold green")
    table.add_column("Metric", style="cyan", no_wrap=True)
    table.add_column("Value", style="yellow")
    
    table.add_row("Total Feasible Transitions", str(len(feasible_transitions_list)))
    table.add_row("Unique Nodes Involved", str(len(unique_nodes)))
    table.add_row("CLIMB Transitions", str(phase_counts["CLIMB"]))
    table.add_row("CRUISE Transitions", str(phase_counts["CRUISE"]))
    table.add_row("DESCENT Transitions", str(phase_counts["DESCENT"]))
    
    if altitude_ranges:
        table.add_row("Min Altitude (ft)", f"{min(altitude_ranges):,.0f}")
        table.add_row("Max Altitude (ft)", f"{max(altitude_ranges):,.0f}")
...
def tres_backward(
    ...
    delta_t_seconds_wall_clock: int = None,
    delta_t_seconds_climb: int = None,
    max_flight_duration_hours: int = None,
    climb_phase_switch_allowance_climb_time_bins: int = None,
    ...
    with Progress(
        ...
        main_task = progress.add_task("Processing topological generations...", total=len(topo_generations_node_ids))
            ...
            for i in range(len(batch_identifiers_list)):
                ...
                if (u_idx, v_idx) in transitions_map_by_indices:
                    ...
                    if phi_v_idx == PHASE_CLIMB: # <--- CLIMB TO CLIMB TRANSITION
                        ...
                        # OLD: eta_u_trans_ssm_val = eta_v_ssm_val - edge_climb_time_seconds - edge_cruise_time_seconds
    ...
    # Calculate processing time and display output summary
    processing_time = time.time() - start_time
    _display_output_summary(feasible_transitions_list, processing_time)

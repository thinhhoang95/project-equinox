...
from equinox.training.prep.graph_scripts.cycle_prune import (
    ...
    for idx, row in route_df.iterrows():
        ...
        for u, v in zip(wpts, wpts[1:]):
            if Gm.has_node(u) and Gm.has_node(v):
                ...
                Gm.add_edge(
                    u, v, dist=dist_nm, t_wind=0.0, pref=0  # default tail‑wind (m/s)
                )  # default preference


def add_edges_from_historical_data(
    directory: str, origin: str, dest: str, graph: nx.Graph
):
    """
...
def prepare_base_graph(
    ...
    Gno = filter_graph_by_detour(
        Gno, source_id, destination_id, minimum_detour_allowed
    )  # Only 7.5% of *minimum* detour is allowed, real path could be much longer
    print(
        f"The filtered graph has {Gno.number_of_nodes()} nodes and {Gno.number_of_edges()} edges"
    )

    # Compute bearing between source and destination
    bearing = compute_bearing(
        Gno.nodes[source_id]["lat"],
        Gno.nodes[source_id]["lon"],
        Gno.nodes[destination_id]["lat"],
        Gno.nodes[destination_id]["lon"],
    )
    ...
    add_edges_from_historical_data(
        directory=routes_dir, origin=source_id, dest=destination_id, graph=Gno
    )
...
def remove_collinear_edges(Gno):
    ...
    for i in range(10):
        ...
        print(
            f"Collinearity check iteration {i}: {Gno.number_of_edges()} edges remaining."
        )
    return Gno
...
def remove_isolated_nodes(Gno):
    ...
    print(
        f"The graph now has {Gno.number_of_nodes()} nodes and {Gno.number_of_edges()} edges."
    )
    return Gno
...
from equinox.training.prep.graph_scripts.improve_connectivity import (
    improve_graph_connectivity,
)
...
def improve_connectivity(Gno, source_id, destination_id, n_iter=20):
    ...
    for i in range(n_iter):
        ...
        print(
            f"Iteration {i}, there are {len(nodes_that_cannot_reach_goal(Gno, destination_id))} nodes that can not reach {destination_id}, {len(nodes_that_source_cannot_reach(Gno, source_id))} nodes that can not be reached from {source_id}"
        )
        edge_before = Gno.number_of_edges()

        Gno = improve_graph_connectivity(
            Gno,
            orphan_goal_nodes,
            orphan_source_nodes,
...
def remove_backtracking_edges(
    G, source_id, destination_id, max_allowed_deviation_angle=90
):
    """
    ...
    """
    main_bearing = compute_bearing(
        G.nodes[source_id]["lat"],
        G.nodes[source_id]["lon"],
        G.nodes[destination_id]["lat"],
        G.nodes[destination_id]["lon"],
    )
    ...
    if edges_to_remove:
        print(
            f"Removed {len(edges_to_remove)} backtracking edges (angle > {max_allowed_deviation_angle}° from main bearing)."
        )
    return G
...
def remove_unreachable_nodes(G, source_id, destination_id):
    ...
    try:
        reachable_from_source = set(
            nx.single_source_shortest_path_length(G, source_id).keys()
        )
    ...
    try:
        can_reach_destination = set(
            nx.single_source_shortest_path_length(G_reversed, destination_id).keys()
        )
    except nx.NetworkXNoPath:
    ...
    if nodes_to_remove:
        ...
        print(
            f"Removed {len(nodes_to_remove)} unreachable nodes. Graph now has {G.number_of_nodes()} nodes."
        )
    else:

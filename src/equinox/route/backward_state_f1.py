...
def get_next_state_bw(
    ...
    alt_s_out = torch.full((num_segments,), torch.nan, device=device, dtype=dtype)
    eta_s_out = torch.full((num_segments,), torch.nan, device=device, dtype=dtype)
    ...
    if len(descent_profile_raw) >= 2:
        ...
        if delta_time_s_cruise > 1e-6: # Avoid division by zero
            ...
            if tas_cruise_kts < 0:
                tas_cruise_kts = -tas_cruise_kts # make sure it's positive
        else:
    ...
    non_cruise_mask = (phase_t_actual == DESCENT) # Changed: Only allow DESCENT phase here
    ...
    if non_cruise_mask.any():
        ...
        if num_nc > 0:
            ...
            # Extracted columns from the descent performance table: altitude, ETATO, and distance wind-free from ToD
            ...
            # distance column of descent performance table, but adjusted by the wind speed at the target node
            ...
            # Interpolate to find profile time and wind-free distance at alt_t_nc (the target node's altitude)
            # => all of these values are relative to the landing point (g)
            # This gives time to landing point (g) and dist_wf to landing point (g)
            ...
            # Ground distance covered from the target point (T) to the landing point (g) (remember: our computation goes backwards, from target back to source, but the flight is from source to target)
            ground_dist_T_from_landing_val = dist_wf_T_profile_val + wind_kts_at_s_nc * (time_T_profile_val / 3600.0)

            # Target total ground distance from the target point (T) to reach the source point (S)
            # Moving backward from T to S, so subtract leg distance.
            target_ground_dist_S_from_landing = ground_dist_T_from_landing_val + dist_leg_ST_nm_nc # needs to be positive because the performance table represents the distance to go (which is positive)
            ...
            # Ground distance from ToD to landing point itself (should be 0 if P_dist_wf[0] and P_time[0] are 0)
            ...
            alt_s_interp = batched_interp1d_torch_anyorder(
                target_ground_dist_S_from_landing, P_ground_dist_prof_nc, P_alt_prof.unsqueeze(0).expand(num_nc, -1), device
            )
            time_S_profile_interp = batched_interp1d_torch_anyorder(
                target_ground_dist_S_from_landing, P_ground_dist_prof_nc, P_time_prof.unsqueeze(0).expand(num_nc, -1), device
            )

            # --- Edge Case: Top of Descent (ToD) ---
            # Mask for segments where S is still descending (after ToD), i.e., S is in the descent path
            is_S_in_descent_mask = target_ground_dist_S_from_landing <= ground_dist_at_ToD_prof
            
            # Mask for segments where ToD is crossed on leg S->T
            # (S is at or before ToD, T was after ToD)
            is_ToD_crossed_mask = (target_ground_dist_S_from_landing >= ground_dist_at_ToD_prof) & \
                                  (ground_dist_T_from_landing_val < ground_dist_at_ToD_prof)
            ...
            if is_ToD_crossed_mask.any():
                ...
                time_desc_ToD_to_T = time_T_profile_val[mask] - P_time_prof[0]
                time_desc_ToD_to_T = torch.clamp(time_desc_ToD_to_T, min=0)

                # Ground distance covered in descent (from ToD to T)
                dist_desc_ToD_to_T_ground = -ground_dist_T_from_landing_val[mask] + ground_dist_at_ToD_prof[mask]
            ...
            # This case (Case 3) should not happen with correct phase labeling, so this is just a fallback.

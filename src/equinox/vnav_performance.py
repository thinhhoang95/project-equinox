...
class Performance:
    ...
    def __init__(self,
        ...
        """
        if not all([climb_speed_profile, descent_speed_profile, climb_vertical_speed_profile, descent_vertical_speed_profile]):
            raise ValueError("All profiles (climb/descent speed and vertical speed) must be provided and non-empty.")

        if not (climb_speed_profile[-1][0] == float('inf') and
                descent_speed_profile[-1][0] == float('inf') and
                climb_vertical_speed_profile[-1][0] == float('inf') and
                descent_vertical_speed_profile[-1][0] == float('inf')):
            raise ValueError("All profiles must end with a float('inf') altitude threshold.")
    ...
    def _get_value_at_alt(self, altitude_ft: float, profile: List[Tuple[float, float]]) -> float:
        """
        ...
        if profile: # If profile was not empty
            return profile[-1][1] # Fallback to last value if somehow loop didn't catch (e.g. alt_ft = inf)
        raise ValueError(f"Could not determine value for altitude {altitude_ft} from profile. Profile might be empty or malformed unexpectedly.")
    ...
    def _create_combined_profile(self,
        ...
        if unique_sorted_boundaries:
            ...
            for i in range(1, len(unique_sorted_boundaries)):
                if unique_sorted_boundaries[i] > unique_sorted_boundaries[i-1]: # Strict increase
                    processed_boundaries.append(unique_sorted_boundaries[i])

        if not processed_boundaries or (len(processed_boundaries) == 1 and processed_boundaries[0] != float('inf')):
             raise ValueError(f"Profile boundaries do not extend to infinity properly or are insufficient: {processed_boundaries}")
    ...
    def _get_values_from_combined_profile(self, altitude_ft: float, combined_profile: List[Tuple[float, float, float]]) \
                                          -> Tuple[float, float]:
        """
        ...
        if combined_profile: # Should be non-empty due to constructor checks
             return combined_profile[-1][1], combined_profile[-1][2] # Value from the last segment (ending in inf)
        
        raise Exception(f"Could not determine performance values for altitude {current_altitude_ft}. Combined profile might be misconfigured or empty.")
    ...
    def get_tas(self, altitude_ft: float, phase: str) -> float:
        ...
        if phase.lower() == 'climb':
            ...
            speed, _ = self._get_values_from_combined_profile(current_altitude_ft, self._combined_climb_profile)
            return speed
        elif phase.lower() == 'descent':
            speed, _ = self._get_values_from_combined_profile(current_altitude_ft, self._combined_descent_profile)
            return speed
        elif phase.lower() == 'cruise':
            return self.cruise_speed_kts
        else:
            raise ValueError(f"Invalid phase: {phase}. Must be 'climb', 'descent', or 'cruise'.")
    ...
    def get_vertical_speed(self, altitude_ft: float, phase: str) -> float:
        ...
        if phase.lower() == 'climb':
            ...
            _, vs = self._get_values_from_combined_profile(current_altitude_ft, self._combined_climb_profile)
            return vs
        elif phase.lower() == 'descent':
            ...
            if current_altitude_ft <= 0.001: # Using a small epsilon for ground check
                 ...
                 return 0.0
            _, vs = self._get_values_from_combined_profile(current_altitude_ft, self._combined_descent_profile)
            return vs # Assumes descent_vertical_speed_profile provides negative values.
        elif phase.lower() == 'cruise':
            return 0.0
        else:
            raise ValueError(f"Invalid phase: {phase}. Must be 'climb', 'descent', or 'cruise'.")

    def get_climb_eta(self, origin_airport_elevation_ft: float) -> List[Tuple[float, float]]:
        """
    ...
    def get_descent_eta(self, destination_airport_elevation_ft: float) -> List[Tuple[float, float]]:
        """
        ...
        target_altitudes_to_pass = sorted(list(set(
            [b for b in descent_profile_boundaries if b < current_alt_ft and b >= destination_alt_ft] + \
            [destination_alt_ft]
        )), reverse=True)
        
        target_altitudes_to_pass = [alt for alt in target_altitudes_to_pass if alt < current_alt_ft]
        ...
        for next_target_alt in target_altitudes_to_pass:
            ...
            if vs_fpm_for_segment is None:
                ...
                _unused_speed, vs_fpm_for_segment = self._get_values_from_combined_profile(current_alt_ft, self._combined_descent_profile)

            if vs_fpm_for_segment is None: # Descent VS must be negative
                break 

            if vs_fpm_for_segment > 0:
                vs_fpm_for_segment = -vs_fpm_for_segment # Convert to negative for descent if the original VS is positive (in case the vnav_profiles.py gives a positive VS for a descent profile)

            actual_descent_target_for_step = next_target_alt

            if current_alt_ft > actual_descent_target_for_step: # Should always be true by loop construction
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Climb phase
        print(f"Climb TAS at 3000 ft: {light_perf.get_tas(3000, 'climb')} kts") # Expected: 80
        print(f"Climb VS at 3000 ft: {light_perf.get_vertical_speed(3000, 'climb')} fpm") # Expected: 600
        
        print(f"Climb TAS at 5000 ft: {light_perf.get_tas(5000, 'climb')} kts") # Expected: 90 (since 5000 is start of next segment)
        print(f"Climb VS at 5000 ft: {light_perf.get_vertical_speed(5000, 'climb')} fpm") # Expected: 400
        
        print(f"Climb TAS at 6000 ft: {light_perf.get_tas(6000, 'climb')} kts") # Expected: 90
        print(f"Climb VS at 6000 ft: {light_perf.get_vertical_speed(6000, 'climb')} fpm") # Expected: 400

        # Reaching cruise altitude
        print(f"Climb TAS at 7000 ft: {light_perf.get_tas(7000, 'climb')} kts") # Expected: 85 (cruise_speed_kts)
        print(f"Climb VS at 7000 ft: {light_perf.get_vertical_speed(7000, 'climb')} fpm") # Expected: 0.0
        print(f"Climb TAS at 8000 ft: {light_perf.get_tas(8000, 'climb')} kts") # Expected: 85 (cruise_speed_kts)
        print(f"Climb VS at 8000 ft: {light_perf.get_vertical_speed(8000, 'climb')} fpm") # Expected: 0.0

        # Cruise phase
        print(f"Cruise TAS at 7000 ft: {light_perf.get_tas(7000, 'cruise')} kts") # Expected: 85
        print(f"Cruise VS at 7000 ft: {light_perf.get_vertical_speed(7000, 'cruise')} fpm") # Expected: 0.0

        # Descent phase
        print(f"Descent TAS at 8000 ft (above cruise, descending from higher): {light_perf.get_tas(8000, 'descent')} kts") # Expected: 85
        print(f"Descent VS at 8000 ft: {light_perf.get_vertical_speed(8000, 'descent')} fpm") # Expected: -300
        
        print(f"Descent TAS at 6000 ft: {light_perf.get_tas(6000, 'descent')} kts") # Expected: 85
        print(f"Descent VS at 6000 ft: {light_perf.get_vertical_speed(6000, 'descent')} fpm") # Expected: -300

        print(f"Descent TAS at 5000 ft: {light_perf.get_tas(5000, 'descent')} kts") # Expected: 85 (start of [5000, inf) segment for descent)
        print(f"Descent VS at 5000 ft: {light_perf.get_vertical_speed(5000, 'descent')} fpm") # Expected: -300

        print(f"Descent TAS at 3000 ft: {light_perf.get_tas(3000, 'descent')} kts") # Expected: 95
        print(f"Descent VS at 3000 ft: {light_perf.get_vertical_speed(3000, 'descent')} fpm") # Expected: -500
        
        print(f"Descent TAS at 0 ft: {light_perf.get_tas(0, 'descent')} kts") # Expected: 95
        print(f"Descent VS at 0 ft: {light_perf.get_vertical_speed(0, 'descent')} fpm") # Expected: 0.0

        # Test with more complex profiles (where thresholds don't align)
        NB_JET_CLIMB_SPEED: List[Tuple[float, float]] = [(10000, 250), (28000, 300), (float('inf'), 450)]
        NB_JET_CLIMB_VS_MODIFIED: List[Tuple[float, float]] = [(5000, 3000), (15000, 2000), (float('inf'), 1200)]
        ...
        print(f"NB Climb VS at 4000 ft: {nb_perf.get_vertical_speed(4000, 'climb')}") # Exp: 3000

        print(f"NB Climb TAS at 7000 ft: {nb_perf.get_tas(7000, 'climb')}")   # Exp: 250
        print(f"NB Climb VS at 7000 ft: {nb_perf.get_vertical_speed(7000, 'climb')}") # Exp: 2000

        print(f"NB Climb TAS at 12000 ft: {nb_perf.get_tas(12000, 'climb')}") # Exp: 300
        print(f"NB Climb VS at 12000 ft: {nb_perf.get_vertical_speed(12000, 'climb')}")# Exp: 2000

        print(f"NB Climb TAS at 20000 ft: {nb_perf.get_tas(20000, 'climb')}")# Exp: 300
        print(f"NB Climb VS at 20000 ft: {nb_perf.get_vertical_speed(20000, 'climb')}")# Exp: 1200

        print(f"NB Climb TAS at 30000 ft: {nb_perf.get_tas(30000, 'climb')}")# Exp: 450
        print(f"NB Climb VS at 30000 ft: {nb_perf.get_vertical_speed(30000, 'climb')}")# Exp: 1200
    ...
    try:
        ...
        eta_descent3 = perf_cruise_low.get_descent_eta(destination_airport_elevation_ft=5000)

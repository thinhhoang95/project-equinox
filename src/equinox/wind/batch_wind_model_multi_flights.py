...
class BatchWindModel(WindModel):
    """
    A wind model that handles batches of flights, loading the necessary
    time-disparate ERA5 data snapshots. It inherits from WindModel to reuse
    interpolation logic but overrides the data loading part.
    """
    def __init__(
        self,
        flight_batch_df: pd.DataFrame,
        ...
        for a specific batch of flights.
        ...
        for f_path in all_files_paths:
            try:
                # Extract timestamp from filename like '1680330385.nc'
                timestamp = int(os.path.splitext(os.path.basename(f_path))[0])
                file_timestamps_map[timestamp] = f_path
            except (ValueError, IndexError):
                # Ignore files that do not have a unix timestamp as a name
                continue
        ...
        # 2. Determine the required minimal set of files for all flights in the batch
        required_file_paths = set()
        buffer_s = time_buffer_h * 3600

        for _, flight in flight_batch_df.iterrows():
            takeoff_time = flight['takeoff']
            
            # Determine time window for this specific flight
            start_time_unix = takeoff_time - buffer_s
            end_time_unix = takeoff_time + typical_flight_duration_s + buffer_s
        ...
        if not files_to_load:
            min_t_dt = datetime.fromtimestamp(flight_batch_df['takeoff'].min())
            max_t_dt = datetime.fromtimestamp(flight_batch_df['takeoff'].max() + typical_flight_duration_s)
            raise FileNotFoundError(f"No suitable data files found for any flight in the batch "

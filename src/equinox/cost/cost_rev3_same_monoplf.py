...
class CostRev3(nn.Module):
    ...
    The formula is:
    c(e, t_e, beta) = beta_0 + beta_1 * PLM_ac(AC(e) * d(e)) + beta_2 * PLM_wind(w_tail(e, t_e)) + beta_3 * P(e)
    ...
    - P(e) is the preference score for edge e.
    """
    def __init__(self,
                 beta0: float,
                 beta1: float,
                 beta2: float,
                 beta3: float, # not used for the moment
                 alpha_pref_reg: float, # Regularization weight for preference scores
                 num_waypoints: int,
        ...
        if isinstance(alpha_pref_reg, torch.Tensor):
            ...
            self.alpha_pref_reg = nn.Parameter(torch.tensor(alpha_pref_reg, dtype=torch.float32), requires_grad=False)
    ...
    def get_preference_score_batched(self, u_indices: torch.Tensor, v_indices: torch.Tensor, preference_matrix_p: torch.Tensor) -> torch.Tensor:
        """
        Retrieves preference scores P(e) for a batch of edges e = (u, v).
        """
        return self._get_edge_metric_batched(u_indices, v_indices, preference_matrix_p)
    ...
    def forward(self,
        ...
        # Preference regularization term (squared L2 norm of preferences for the batch)
        # This penalizes large preference values to prevent overfitting to preferences.

...
def test_wind_model():
    ...
    u, v = wind_model.get_wind_components(lat=48.8566, lon=2.3522, alt_ft=50.0, time=datetime(2024, 4, 1, 12, 0, 0))
    ...
    u, v = wind_model.get_wind_components(lat=48.8566, lon=2.3522, alt_ft=1000.0, time=datetime(2024, 4, 1, 12, 0, 0))
    assert u == u # Check if u is not NaN
    assert v == v # Check if v is not NaN
    print('Wind at 48.8566N, 2.3522E (Paris), 1000ft, 12:00:00 is', u, v)

    u, v = wind_model.get_wind_components(lat=36.7213, lon=-4.4214, alt_ft=30000.0, time=datetime(2024, 4, 1, 12, 0, 0))
    assert u == u # Check if u is not NaN
    assert v == v # Check if v is not NaN
    print('Wind at 36.7213N, -4.4214E (Malaga), 30000ft, 12:00:00 is', u, v)

    u, v = wind_model.get_wind_components(lat=48.8566, lon=2.3522, alt_ft=30000.0, time=datetime(2024, 4, 1, 12, 0, 0))
    assert u == u # Check if u is not NaN
    assert v == v # Check if v is not NaN
...
def test_cape_cin():
    ...
    data_dir = os.path.join(project_root, 'data', 'era5')
    wind_model = WindModel(date_str="2024-04-01", data_dir=data_dir)
    cape, cin = wind_model.get_cape_cin(lat=48.8566, lon=2.3522, time=datetime(2024, 4, 1, 12, 0, 0))

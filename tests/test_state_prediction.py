...
def test_cruise_only_wind_free():
    # Create a wind-free wind model
    wind_model = WindFree()
    ...
    # Create source state
    coords_src = torch.tensor([[37.7749, -122.4194]]) # SFO
    alts_src = torch.tensor([35000]) # 35000 ft
    eta_src = torch.tensor([0]) # 0 seconds
...
def test_one_climb_phase_wind():
    ...
    # Departure time
    time_at_departure = "2024-04-01 12:00:00"
    seconds_since_midnight = datestr_to_seconds_since_midnight(time_at_departure)
    
    # Create source state
    coords_src = torch.tensor([[37.7749, -122.4194]]) # SFO
    alts_src = torch.tensor([1000]) # 1000 ft, elevation at departure
    eta_src = torch.tensor([seconds_since_midnight]) # seconds since midnight
    phase_src = torch.tensor([0]) # CLIMB
...
def test_one_descent_phase_no_wind():
    ...
    coords_src = torch.tensor([[41.316, -76.358]]) # MAGIO waypoint

    # Get the haversine distance between the source and target
    dist_nm = haversine(coords_src[0, 0], coords_src[0, 1], coords_tgt[0, 0], coords_tgt[0, 1])

    print(f"Distance between MAGIO and KJFK: {dist_nm:.2f} nautical miles")
